...
} from "../entities/utils/chain-id-mapping";
import { dedupFeesTokens } from "../entities/utils/dedupe-fee-tokens";
import { submitSolanaTransaction } from "../entities/submit-solana-transaction";

const log = Debug("log:adapters:DeBridgeBridgeAdapter");
const debug = Debug("debug:adapters:DeBridgeBridgeAdapter");
...
export class DeBridgeBridgeAdapter extends AbstractBridgeAdapter {
  ...
  protected QUOTE_URL = "https://api.dln.trade/v1.0/dln/order/quote";
  ...
  async getSwapDetails(
    ...
    const quoteUrl = new URL(this.QUOTE_URL);
